...
export function usePools() {
  ...
  const poolMap = useMemo(
    () => new Map(pools.map((p) => [p.coinType, p])),
    [pools]
  );
...
export function useTokenExchange() {
  ...
  const handleMergeSplit = useCallback(
    (tx: Transaction, coinObjects: any[], amount: bigint) => {
  ...
  const handleAddtoLiquidity = useCallback(
    async (coinType: string, amount: number) => {
      ...
      const liqObj = await getObject(
        address,
        `${clientConfig.PACKAGE_ID}::pool::PayfricaPoolTicket`
      );
      if (liqObj === null) {
        tx.moveCall({
  ...
  const handleRemoveLiquidity = useCallback(
    async (coinType: string, amount: number) => {
      ...
      tx.moveCall({
        ...
        typeArguments: [a.coinType],
        arguments: [tx.object(a.id), tx.object(liqObj), tx.pure.u64(amt)],
      });
      return sponsorAndExecuteTransactionBlock({
        tx,
        network: clientConfig.SUI_NETWORK_NAME,
  ...
  const handleWithdrawalRequest = useCallback(
    async (coinType: string, amount: number, agent: string) => {
      if (!address) throw new Error("No wallet");
      const tx = new Transaction();
  ...
  const handleCreateCard = useCallback(
    async (
      ...
      if (!address) throw new Error("No wallet");
      const payfObj = await getObject(
        address,
        `${clientConfig.PACKAGE_ID}::payfrica::PayfricaUser`
      );
      if (payfObj === null) {
  ...
  const sendToAddress = useCallback(
    async (coinType: string, amount: number, recipient: string) => {
      if (!address) throw new Error("No wallet");
      const t = poolMap.get(coinType)!;
  ...
  const sendToNameService = useCallback(
    async (coinType: string, amount: number, ns: string) => {
      if (!address) throw new Error("No wallet");
      const t = poolMap.get(coinType)!;
      const amt = toMinimalUnits(amount, t.coinDecimal);
      const tx = new Transaction();
  ...
  const getAllPools = useCallback(async () => {
    ...
    const result = await Promise.all(
      Array.from(poolMap.values()).map(async (pool) => {
        ...
        const balance =
          Number(balanceRaw.totalBalance) / 10 ** pool.coinDecimal;
  ...
  const getPortfolio = useCallback(async () => {
    ...
    const local = poolsArr.find(
      (p) => p.coinType === userDetails?.country.baseTokencoinType
    );
